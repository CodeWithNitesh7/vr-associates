...
import { getFrontClients } from "../api/Services/clientApi";
...
export default function Clients() {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      setLoading(true);
      const data = await getFrontClients();
      setClients(data || []);
      setLoading(false);
    };
  ...
  if (loading) {
    return (
      <div className="py-20 text-center text-xl text-sky-500 font-semibold">
        Loading clients...
      </div>
    );
  }
  ...
  if (!loading && clients.length === 0) {
    return (
      <div className="py-20 text-center text-xl text-gray-600 font-semibold">
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-10 text-center">
        ...
        <p className="max-w-3xl mx-auto text-gray-700 text-base sm:text-lg leading-relaxed mb-12">
          We are proud to collaborate with some of the most respected global
          brands — helping them achieve their staffing and growth goals through
          ...
          {clientsToDisplay.map((client, index) => (
            ...
            >
              ...
              <div className="flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 mb-4 rounded-full bg-white p-3 shadow-inner">
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src =
                      "https://placehold.co/96x96/cccccc/000000?text=Logo";
                  }}
                />
              </div>
              ...
              <p className="text-gray-600 text-sm italic mt-3 leading-snug h-20 overflow-hidden">
                “{client.testimonial.slice(0, 80)}
                {client.testimonial.length > 80 ? "..." : ""}”
              </p>
