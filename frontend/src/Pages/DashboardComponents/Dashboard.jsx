...
// Updated navigation structure - 'current' property removed as state will manage the active page
const navigation = [
    { name: 'Dashboard', icon: HomeIcon, href: '#' },
    { name: 'Job Listings', icon: BriefcaseIcon, href: '#' },
    { name: 'Candidates', icon: UsersIcon, href: '#' },
    { name: 'Clients & Companies', icon: BuildingOffice2Icon, href: '#' },
    { name: 'Analytics & Reports', icon: ChartBarIcon, href: '#' },
        ...
        <div className="flex h-screen bg-gray-50 font-sans">
                        ...
                        <nav className="mt-5 px-2 space-y-1">
                            {navigation.map((item) => (
                                <a
                                    ...
                                    className={`group flex items-center px-2 py-2 text-base font-medium rounded-md transition duration-150 ease-in-out ${currentPage === item.name
            ...
            <div className="hidden lg:flex lg:flex-shrink-0">
                <div className="flex flex-col w-64 bg-gray-900 h-screen shadow-2xl">
                    <div className="p-6 text-2xl font-extrabold text-indigo-400 tracking-wider border-b border-gray-700">
                        VR Associates
                    </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                {/* Header/Top Bar */}
                <header className="flex-shrink-0 flex items-center justify-between h-16 bg-white shadow-md border-b border-gray-200 px-4 sm:px-6 lg:px-8">
                ...
                <main className="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-8">
                    {/* Display the current page content title based on state */}
                    <h1 className="text-3xl font-extrabold text-gray-900 capitalize mb-6">
                        {currentPage}
                    </h1>

                    {/* Conditional content based on currentPage state (currently shows only Dashboard) */}
                    {currentPage === 'Dashboard' && (
                        <>
                            ...
                            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-md border border-gray-100 flex flex-col h-96">
                                    <h2 className="text-xl font-bold text-gray-800 mb-4">Recruitment Pipeline Funnel</h2>
                                    <p className="text-gray-500 mb-4">
                                        This section is reserved for the visual chart integration.
                                    </p>
                                    <div className="w-full flex-grow bg-indigo-50 rounded-lg flex items-center justify-center border-dashed border-2 border-indigo-200">
                                        <span className="text-indigo-400 text-lg font-medium">CHART VISUALIZATION AREA (Recharts Integration)</span>
                                    </div>
                                </div>
                            </section>
                        </>
                    ...
                    {currentPage !== 'Dashboard' && (
                        <div className="p-8 bg-white rounded-xl shadow-md border border-gray-100 min-h-[40vh] flex items-center justify-center">
                            <p className="text-xl text-gray-500 font-medium">
                                Content for the <span className="text-indigo-600 font-bold">{currentPage}</span> page would go here.
                            </p>
                        </div>
                    )}
